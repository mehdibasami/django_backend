...
from account.serializers import GmailSerializer, RegisterSerializer, RegisteredUserSerializer, LoginSerializer, UserInfoSerializer, UserProfileUpdateSerializer
from fitness_backend.utils.custom_serializers import create_response_serializer
from fitness_backend.utils.response_state import SuccessResponse, BadRequestResponse, ServerErrorResponse, SuccessResponse201
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                if response_info.status_code != 200:
                    return BadRequestResponse(
                        message="Invalid access token or user information could not be retrieved"
                    )
                user_info = response_info.json()
                name = user_info['name']
                picture_link = user_info['picture']
                ...
                if not user:
                    ...
                    user.set_password(str(user.reset_password_token))
                    if picture_link:
                        temp_file = NamedTemporaryFile(delete=True)
                        temp_file.write(pic_response.content)
                        user.profile_picture.save(
                            f"{user.name}_{user.email}.png",
                            File(temp_file),
                            save=True
                        )
                    user.save()
